...
def test_filter_by_state(filter_state: str) -> list:
...
def test_filter_by_state_different_meanings() -> str:
    """Тест при отсутствии словарей с указанным статусом state в списке, и для различных возможных значений статуса state."""
    assert filter_by_state(state="DEVAISE") == []
    assert filter_by_state(state="1234") == []
    assert filter_by_state(state="hello") == []


def test_sort_by_date(date: list, sorted_list_descending: list) -> list:
    """Тестирование сортировки списка словарей по датам в порядке убывания"""
    assert sort_by_date(date) == sorted_list_descending


def test_sort_by_identical_date(sorted_list_identical: list) -> list:
...
def test_sort_by_date_not_standart(sorted_list_not_standard: list) -> list:
